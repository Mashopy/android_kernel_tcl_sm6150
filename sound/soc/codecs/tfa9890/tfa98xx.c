 ...
 */
//#define DEBUG // MODIFIED by hongwei.tian, 2020-01-07,BUG-8743798
...
#include <sound/tfa_mmi_test.h> // MODIFIED by hongwei.tian, 2019-05-10,BUG-7754996
...
static int pcm_no_constraint = 1; // MODIFIED by hongwei.tian, 2019-08-07,BUG-8207233
...
/* MODIFIED-BEGIN by hongwei.tian, 2019-05-10,BUG-7754996*/
#ifdef CONFIG_SND_SOC_TFA98XX_MMI_TEST
struct tfa98xx *ext_tfa98xx;
static unsigned int  tfa_mi2s_interface = 0;
extern void tfa98xx_sysfs_init(void);

unsigned int tfa_get_mi2s_interface(void)
{
	return tfa_mi2s_interface;
}
EXPORT_SYMBOL_GPL(tfa_get_mi2s_interface);
#endif
/* MODIFIED-END by hongwei.tian,BUG-7754996*/
...
static struct snd_soc_dai_driver tfa98xx_dai[] = {
	{
		...
		.symmetric_channels = 0, // MODIFIED by hongwei.tian, 2020-02-26,BUG-8953137
...
int tfa98xx_ext_reset(struct tfa98xx *tfa98xx) // MODIFIED by hongwei.tian, 2019-05-10,BUG-7754996
{
	...
	if ((no_start == 0) && (no_reset == 0)) {
		...
		if (ret < 0) {
			dev_err(&i2c->dev, "Failed to read Revision register: %d\n",
				ret);
			i2c_check_status_create("audio_smpart_pa",0); // MODIFIED by hongwei.tian, 2020-01-07,BUG-8743798
			return -EIO;
		}
		switch (reg & 0xff) {
		case 0x72: /* tfa9872 */
...
/* MODIFIED-BEGIN by hongwei.tian, 2019-05-10,BUG-7754996*/
	...
	ret = of_property_read_string(np,
				"tfa,mi2s-interface", &intf_name);
	if (!ret)
	{
		if (!strcmp(intf_name, "primary")) {
			tfa_mi2s_interface = 1;
		} else if (!strcmp(intf_name, "secondary")) {
			tfa_mi2s_interface = 2;
		} else if (!strcmp(intf_name, "tertiary")) {
			tfa_mi2s_interface = 3;
		} else if (!strcmp(intf_name, "quaternary")) {
			tfa_mi2s_interface = 4;
		} else {
			dev_err(&i2c->dev, "%s: invalid DT intf name %s\n",
				__func__, intf_name);
		}
	}
	else{
		if(!tfa_mi2s_interface)
			dev_err(&i2c->dev,"%s: tfa,mi2s-interface missing in DT node\n",
				__func__);
	}
...
/* MODIFIED-END by hongwei.tian,BUG-7754996*/
	...
	mutex_unlock(&tfa98xx_mutex);
	i2c_check_status_create("audio_smpart_pa",1); // MODIFIED by hongwei.tian, 2020-01-07,BUG-8743798
	return 0;
}
