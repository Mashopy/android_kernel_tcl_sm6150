...
char *tfaContBitName(uint16_t num, unsigned short rev)
{
	...
	case 0x94:
	   if (((rev >>8) & 0xff) == 0x2a || ((rev >> 8) & 0xff) == 0x3a)
		name = tfa_bf2name(Tfa9894N2BitNames, num);
	   else
...
char *tfaContDsName(uint16_t num, unsigned short rev)
{
	...
	case 0x94:
	  if (((rev >>8) & 0xff) == 0x2a || ((rev >> 8) & 0xff) == 0x3a)
		name =  tfa_bf2name(Tfa9894N2DatasheetNames, num);
	   else
...
uint16_t tfaContBfEnum(const char *name, unsigned short rev)
{
	...
	case 0x94:
		if (((rev >> 8) & 0xff) == 0x2a || ((rev >> 8) & 0xff) == 0x3a)
		{
			bfnum = tfa_name2bf(Tfa9894N2DatasheetNames, name);
			if (bfnum == 0xffff)
				bfnum = tfa_name2bf(Tfa9894N2BitNames, name);/* try long bitname table */
		}
		else
		{
...
uint16_t tfaContBfEnumAny(const char *name)
{
	...
	if (bfnum!=0xffff)
		return bfnum;
    bfnum = tfa_name2bf(Tfa9894N2BitNames, name);
	if (bfnum != 0xffff)
		return bfnum;
