...
#define IRIS_COMPATIBLE_NAME  "pixelworks,iris3"
...
static int iris_i2c_send_msg(struct i2c_msg *msgs, int len)
{
	...
	//IRIS_LOGE("iris come here send msg\n");
	while (retry < 3) {
		ret = i2c_transfer(client->adapter, msgs, len);
		if (ret < 1) {
			retry++;
			IRIS_LOGE("iris meet with error when send i2c msg ret = %d\n", ret);
	...
	if (retry == 3) {
		IRIS_LOGE("iris can not transfer msgs\n");
...
static int iris_i2c_rw_t(uint32_t type, struct addr_val *val, int len)
{
	...
	switch (type) {
		...
		case FOUR_BYTE_REG_LEN_WRITE:
			...
			break;
		default:
			IRIS_LOGE("can not identify the cmd = %x\n", type);
			return -EINVAL;
	}
	return ret;
}
...
static int iris_i2c_rw(uint32_t type, struct addr_val *val, int len)
{
	...
	if (!client) {
		IRIS_LOGE("iris i2c handle is NULL \n");
		return -EACCES;
	}

	if (!val || len == 0) {
		IRIS_LOGE("the return buf = %p or len = %d \n",
				val, len);
		return -EINVAL;
	}
...
static int iris_i2c_single_conver_ocp_read(uint32_t *ptr, uint32_t len)
{
	...
	if (NULL == p) {
		IRIS_LOGE("[iris5] %s: allocate memory fails\n", __func__);
		return -EINVAL;
	}
...
static int iris_i2c_burst_conver_ocp_read(uint32_t *ptr, uint32_t dlen)
{
...
#if 0
	if (!ptr || dlen < 2) {
		IRIS_LOGE("the parameter is not right\n");
		return -EINVAL;
	}
#endif
	start_addr = *ptr;
	...
	if (ret == 2) {
		...
		ret = ret < 0 ? ret : -EIO;
		IRIS_LOGE("[iris5] %s: i2c_transfer failed, ret=%d\n", __func__, ret);
	}
...
int iris_i2c_single_conver_ocp_write(uint32_t *arr, uint32_t dlen)
{
	...
	ret = iris_i2c_single_write((struct addr_val *)p, dlen);
	kfree(p);
	p = NULL;
...
int iris_i2c_burst_conver_ocp_write(uint32_t base_addr, uint32_t *arr, uint32_t dlen)
{
	...
	iris_payload = kmalloc(sizeof(iris_payload[0]) * msg_len, GFP_KERNEL);
	if (NULL == iris_payload) {
		IRIS_LOGE("[iris3] %s: allocate memory fails\n", __func__);
		return -EINVAL;
	}
	...
	if (ret == 1) {
		...
		ret = ret < 0 ? ret : -EIO;
		IRIS_LOGE("[iris3] %s: i2c_transfer failed, ret=%d\n", __func__, ret);
	}
...
int iris_i2c_group_write(struct iris_i2c_msg * iris_i2c_msg, uint32_t iris_i2c_msg_num)
{
	...
	if ( NULL == msgs ) {
		IRIS_LOGE("[iris3] %s:%d: allocate memory fails\n", __func__,__LINE__);
		return -EINVAL;
	}
...
int iris_i2c_bus_init(void)
{
	...
	if(ret!=0)
		IRIS_LOGE("i2c add driver fail: %d \n", ret );
	return 0;
}
