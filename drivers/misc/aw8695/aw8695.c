 ...
 */
//#define DEBUG // MODIFIED by hongwei.tian, 2019-12-24,BUG-8699290
...
#define OP_AW_DEBUG // MODIFIED by hongwei.tian, 2019-12-20,BUG-8699290
...
extern int i2c_check_status_create(char *name,int value); // MODIFIED by hongwei.tian, 2020-01-07,BUG-8743798
        ...
        struct aw8695_container *aw8695_cont)
{
    ...
    aw8695_i2c_write(aw8695, AW8695_REG_FIFO_AEH,
                    (unsigned char)((aw8695->ram.base_addr>>1)>>8)); /*1/2 FIFO */
    aw8695_i2c_write(aw8695, AW8695_REG_FIFO_AEL,
                    (unsigned char)((aw8695->ram.base_addr>>1)&0x00FF));/*1/2 FIFO*/
    ...
    for(i=shift; i<aw8695_cont->len; i++) {
        aw8695->ramupdate_flag=aw8695_i2c_write(aw8695, AW8695_REG_RAMDATA, aw8695_cont->data[i]);
    }
...
static int aw8695_haptic_juge_RTP_is_going_on(struct aw8695 *aw8695)
{
    ...
    if((reg_val&AW8695_BIT_SYSCTRL_PLAY_MODE_RTP)&&(!(reg_val&AW8695_BIT_SYSCTRL_STANDBY)))
...
{
    unsigned char reg_val = 0;
    unsigned int cnt = 10;//100; // MODIFIED by hongwei.tian, 2019-12-24,BUG-8699290
...
{
    pr_debug("%s enter\n", __func__);
/* MODIFIED-BEGIN by hongwei.tian, 2019-12-20,BUG-8699290*/
#ifdef OP_AW_DEBUG
    do_gettimeofday(&aw8695->t_stop);
    aw8695->game_microsecond = (aw8695->t_stop.tv_sec-aw8695->t_start.tv_sec) * 1000000 + (aw8695->t_stop.tv_usec-aw8695->t_start.tv_usec);
    pr_debug("%s: haptic_audio_delay=%dus\n", __func__, aw8695->game_microsecond);
    if(aw8695->game_microsecond <= 2000) { //if aw8695->game_microsecond < 2000us
        usleep_range(2000,2100);
    }
#endif
/* MODIFIED-END by hongwei.tian,BUG-8699290*/
...
/* MODIFIED-BEGIN by hongwei.tian, 2019-12-20,BUG-8699290*/
#ifdef OP_AW_DEBUG
    do_gettimeofday(&aw8695->t_start);
#endif
/* MODIFIED-END by hongwei.tian,BUG-8699290*/
    return 0;
}
...
static int aw8695_rtp_trim_lra_calibration(struct aw8695 *aw8695)
{
    ...
    if(aw8695->theory_time < aw8695->microsecond) {
        ...
        if((aw8695->microsecond - aw8695->theory_time) > (aw8695->theory_time*cali_err_thr/1000)) {
            return 0;
        }
        /* diff less 0.1%, no need to cali */
        if((aw8695->microsecond - aw8695->theory_time) < (aw8695->theory_time*cali_min_thr/1000)) {
            return 0;
        }
        real_code = ((aw8695->microsecond - aw8695->theory_time)* 4000) / aw8695->theory_time;
        real_code = ((real_code%10 < 5)? 0 : 1) + real_code/10;
        real_code = 32 + real_code;
    }
    if(aw8695->theory_time > aw8695->microsecond) {
        /* diff over 2%, test data error */
        if((aw8695->theory_time - aw8695->microsecond) > (aw8695->theory_time*cali_err_thr/1000)) {
            return 0;
        }
        /* diff less 0.1%, no need to cali */
        if((aw8695->theory_time - aw8695->microsecond) < (aw8695->theory_time*cali_min_thr/1000)) {
            return 0;
        }
        real_code = ((aw8695->theory_time - aw8695->microsecond)* 4000) / aw8695->theory_time ;
        real_code = ((real_code%10 < 5)? 0 : 1) + real_code/10;
        real_code = 32 - real_code;
    }
    ...
    if(lra_rtim_code > 0) {
        aw8695_i2c_write(aw8695, AW8695_REG_TRIM_LRA, (char)lra_rtim_code);
    }
    return 0;
}
...
static int aw8695_rtp_osc_calibration(struct aw8695 *aw8695)
{
    ...
    ret = request_firmware(&rtp_file,
            aw8695_rtp_name[/*aw8695->rtp_file_num*/ 0],
            aw8695->dev);
    if(ret < 0)
    {
        pr_err("%s: failed to read %s\n", __func__,
    ...
    pr_info("%s: rtp file [%s] size = %d\n", __func__,
            aw8695_rtp_name[/*aw8695->rtp_file_num*/ 0], aw8695_rtp->len);
    ...
    if(fre_val == 3) {
        aw8695->theory_time = (aw8695->rtp_len / 12000) * 1000000; /*12K */
    } else if(fre_val == 2) {
        aw8695->theory_time = (aw8695->rtp_len / 24000) * 1000000; /*24K */
    } else if(fre_val == 1 || fre_val == 0) {
        aw8695->theory_time = (aw8695->rtp_len / 48000) * 1000000; /*48K */
    }
	printk("%s aw8695->theory_time = %d\n",__func__,aw8695->theory_time);
        ...
        if (!aw8695_haptic_rtp_get_fifo_afi(aw8695)) {
            ...
            if (aw8695->rtp_cnt != aw8695_rtp->len) {
                ...
                aw8695->rtpupdate_flag =
                        aw8695_i2c_writes(aw8695,AW8695_REG_RTP_DATA, &aw8695_rtp->data[aw8695->rtp_cnt], buf_len);
                aw8695->rtp_cnt += buf_len;
            }
             mutex_unlock(&aw8695->rtp_lock);
        }
	...
	if(aw8695->rtp_cnt == aw8695_rtp->len) {
            ...
            pr_debug("%s playback done aw8695->rtp_cnt= %d \n", __func__,aw8695->rtp_cnt);
            break;
        } else {
            do_gettimeofday(&aw8695->end);
	}

        aw8695->microsecond = (aw8695->end.tv_sec - aw8695->start.tv_sec)*1000000 +
        (aw8695->end.tv_usec - aw8695->start.tv_usec);
        if (aw8695->microsecond > (aw8695->theory_time + (aw8695->theory_time / 20))) {
            pr_debug("%s over time out aw8695->rtp_cnt %d osc_int_state %02x\n", __func__,aw8695->rtp_cnt, osc_int_state);
            break;
          }
    }
    ...
    aw8695->microsecond = (aw8695->end.tv_sec - aw8695->start.tv_sec)*1000000 +
        (aw8695->end.tv_usec - aw8695->start.tv_usec);
    /*calibration osc*/
    pr_debug("%s awinic_microsecond:%ld \n",__func__,aw8695->microsecond);
    pr_debug("%s exit\n", __func__);
 ...
 *
 *****************************************************/
static int aw8695_haptic_audio_ctr_list_insert(
...
static void aw8695_haptic_audio_work_routine(struct work_struct *work)
{
	...
	memset(&aw8695->haptic_audio.ctr, 0,
	...
	if(ctr_list_flag == 1) {
		...
		if(ctr_list_diff_cnt > 2) {
			list_for_each_entry_safe_reverse(p_ctr, p_ctr_bak, &(haptic_audio->ctr_list), list) {
				if((p_ctr->play == 0) &&
					(AW8695_HAPTIC_CMD_ENABLE == (AW8695_HAPTIC_CMD_HAPTIC & p_ctr->cmd))) {
					list_del(&p_ctr->list);
					kfree(p_ctr);
					ctr_list_del_cnt ++;
				}
    ...
    mutex_unlock(&aw8695->haptic_audio.lock);
    if(AW8695_HAPTIC_CMD_ENABLE == aw8695->haptic_audio.ctr.cmd) {
        if(AW8695_HAPTIC_PLAY_ENABLE == aw8695->haptic_audio.ctr.play) {
            ...
            aw8695_haptic_start(aw8695);
            mutex_unlock(&aw8695->lock);
        } else if(AW8695_HAPTIC_PLAY_STOP == aw8695->haptic_audio.ctr.play) {
            mutex_lock(&aw8695->lock);
            aw8695_haptic_stop(aw8695);
            mutex_unlock(&aw8695->lock);
        } else if(AW8695_HAPTIC_PLAY_GAIN == aw8695->haptic_audio.ctr.play) {
            mutex_lock(&aw8695->lock);
            aw8695_haptic_set_gain(aw8695,
                    aw8695->haptic_audio.ctr.gain);
            mutex_unlock(&aw8695->lock);
...
static int aw8695_haptic_cont(struct aw8695 *aw8695)
{
    ...
    /* TD time */
    aw8695_i2c_write(aw8695, AW8695_REG_TD_H, (unsigned char)(aw8695->info.cont_td>>8));
    aw8695_i2c_write(aw8695, AW8695_REG_TD_L, (unsigned char)(aw8695->info.cont_td>>0));
    aw8695_i2c_write(aw8695, AW8695_REG_TSET, aw8695->info.tset);

    /* zero cross */
    aw8695_i2c_write(aw8695, AW8695_REG_ZC_THRSH_H, (unsigned char)(aw8695->info.cont_zc_thr>>8));
    aw8695_i2c_write(aw8695, AW8695_REG_ZC_THRSH_L, (unsigned char)(aw8695->info.cont_zc_thr>>0));
    ...
    aw8695_i2c_write(aw8695, AW8695_REG_DRV_LVL_OV, aw8695->info.cont_drv_lvl_ov);
...
static int aw8695_haptic_f0_calibration(struct aw8695 *aw8695)
{
    ...
    if(aw8695_haptic_get_f0(aw8695)) {
        ...
        /* max and min limit */
        f0_limit = aw8695->f0;
        if(aw8695->f0*100 < aw8695->info.f0_pre*(100-aw8695->info.f0_cali_percen)) {
        ...
        /* calculate cali step */
        f0_cali_step = 100000*((int)f0_limit-(int)aw8695->info.f0_pre)/((int)f0_limit*25);
              pr_debug("%s  line=%d f0_cali_step=%d\n", __func__, __LINE__,f0_cali_step);
              pr_debug("%s line=%d  f0_limit=%d\n", __func__, __LINE__,(int)f0_limit);
              pr_debug("%s line=%d  aw8695->info.f0_pre=%d\n", __func__,__LINE__, (int)aw8695->info.f0_pre);
...
void aw8695_transport_f0_data(unsigned int data)
{
#ifdef AW_UEFI_CAL_F0
    aw8695_f0_cal = data;
#endif
...
static ssize_t aw8695_osc_cali_store(struct device *dev, struct device_attribute *attr,
        const char *buf, size_t count)
{
#ifdef TIMED_OUTPUT
    struct timed_output_dev *to_dev = dev_get_drvdata(dev);
        ...
        const char *buf, size_t count)
{
	...
	if (hap_ctr== NULL ) {
        ...
        aw8695_haptic_audio_ctr_list_insert(&aw8695->haptic_audio, hap_ctr);
...
static void aw8695_vibrator_work_routine(struct work_struct *work)
{
    ...
    aw8695_haptic_stop(aw8695);
    if(aw8695->state) {
        pr_debug("%s aw8695->activate_mode = %d \n", __func__,aw8695->activate_mode); // MODIFIED by hongwei.tian, 2019-12-20,BUG-8699290
        ...
        hrtimer_start(&aw8695->timer,
                  ktime_set(aw8695->duration / 1000, (aw8695->duration % 1000) * 1000000),
                  HRTIMER_MODE_REL);
      }
    mutex_unlock(&aw8695->lock);
}
    ...
    if(reg_val & AW8695_BIT_SYSINT_FF_AEI) {
        ...
        if(aw8695->rtp_init) {
            while((!aw8695_haptic_rtp_get_fifo_afi(aw8695)) &&
                    (aw8695->play_mode == AW8695_HAPTIC_RTP_MODE)) {
                mutex_lock(&aw8695->rtp_lock);
                pr_debug("%s: aw8695 rtp mode fifo update, cnt=%d\n",
                        __func__, aw8695->rtp_cnt);
                if (!aw8695_rtp) {
                   pr_debug("%s:aw8695_rtp is null break\n",
                        __func__);
                   mutex_unlock(&aw8695->rtp_lock);
                   break;
                }
            ...
            pr_debug("%s: aw8695 rtp init = %d, init error\n", __func__, aw8695->rtp_init);
        }
    }
...
static int aw8695_parse_dt(struct device *dev, struct aw8695 *aw8695,
    ...
    val = of_property_read_u32(np, "vib_f0_cali_percen", &aw8695->info.f0_cali_percen);
    if (val != 0)
        printk("vib_f0_cali_percen not found\n");
    val = of_property_read_u32(np, "vib_cont_drv_lev", &aw8695->info.cont_drv_lvl);
    if (val != 0)
        printk("vib_cont_drv_lev not found\n");
    val = of_property_read_u32(np, "vib_cont_drv_lvl_ov", &aw8695->info.cont_drv_lvl_ov);
    if (val != 0)
    ...
    val = of_property_read_u32(np, "vib_cont_zc_thr", &aw8695->info.cont_zc_thr);
    if (val != 0)
        printk("vib_cont_zc_thr not found\n");
    val = of_property_read_u32(np, "vib_cont_num_brk", &aw8695->info.cont_num_brk);
    if (val != 0)
...
memcpy(aw8695->info.f0_trace_parameter, f0_trace_parameter, sizeof(f0_trace_parameter));
...
static int aw8695_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    if (ret < 0) {
        ...
        i2c_check_status_create("viber_aw8695",0); // MODIFIED by hongwei.tian, 2020-01-07,BUG-8743798
        goto err_id;
    }
    ...
    i2c_check_status_create("viber_aw8695",1); // MODIFIED by hongwei.tian, 2020-01-07,BUG-8743798
...
static int __maybe_unused aw8695_suspend(struct device *dev)
{
    int ret = 0;
    struct aw8695 *aw8695 = dev_get_drvdata(dev);
    mutex_lock(&aw8695->lock);
...
static int __maybe_unused aw8695_resume(struct device *dev)
{
    int ret = 0;
    return ret;
}
...
/* MODIFIED-BEGIN by hongwei.tian, 2020-03-11,BUG-8978935*/
#ifdef CONFIG_PM_SLEEP
		.pm = &aw8695_pm_ops,
#endif
/* MODIFIED-END by hongwei.tian,BUG-8978935*/
    },
