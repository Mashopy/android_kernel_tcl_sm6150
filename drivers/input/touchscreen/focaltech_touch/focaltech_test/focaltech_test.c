...
static void fts_test_save_data_csv(struct fts_test *tdata)
{
	...
	sprintf(file_name, "%s-%d%02d%02d%02d%02d%02d.csv", FTS_CSV_FILE_NAME, 
			tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
			tm.tm_hour, tm.tm_min, tm.tm_sec);
    FTS_TEST_INFO("csv length:%d", csv_length);
    fts_test_save_test_data(file_name, csv_buffer, csv_length);
...
static void fts_test_save_result_txt(struct fts_test *tdata)
{
	...
	memset(file_name, 0, sizeof(file_name));
    sprintf(file_name, "%s-%d%02d%02d%02d%02d%02d.txt", FTS_TXT_FILE_NAME, 
			tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
			tm.tm_hour, tm.tm_min, tm.tm_sec);

    FTS_TEST_INFO("test result length in txt:%d", tdata->testresult_len);
    fts_test_save_test_data(file_name, tdata->testresult,
    ...
    struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    count = snprintf(buf, PAGE_SIZE, "TEST RESULT:%s\n",
                     g_test_result ? "Pass" : "Fail");
