...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
	...
	if (IS_ERR_OR_NULL(ts_data->vdd_io)) {
		...
		FTS_ERROR("get vdd_io regulator failed,ret=%d", ret);
	} else {
		if (regulator_count_voltages(ts_data->vdd_io) > 0) {
			ret = regulator_set_voltage(ts_data->vdd_io, 1800000, 1800000);
			if (ret) {
				FTS_ERROR("vdd_io regulator set_vtg failed ret=%d", ret);
				regulator_put(ts_data->vdd_io);
			}
		}
		ret = regulator_enable(ts_data->vdd_io);
		if (ret) {
			FTS_ERROR("enable vdd_io regulator failed,ret=%d", ret);
		}
	}
...
int fts_esd_check(void)
{
    int ret = 0;
    u8 reg_value = 0;

    ret = fts_read_reg(FTS_CMD_READ_ID, &reg_value);
    if (ret < 0)
        pr_err("%s i2c read failed, ret:%d", __func__, ret);
	else
		pr_debug("%s i2c read sucess, reg_value:%d", __func__, reg_value);
