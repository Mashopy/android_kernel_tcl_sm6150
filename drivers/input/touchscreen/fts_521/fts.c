...
#define KEY_GESTURE_D  249
#define KEY_GESTURE_S  250
#define KEY_GESTURE_L  251
...
#define FTS_NORMAL_MODE 1
#define FTS_SUSPEND_MODE 2
#define FTS_LP_MODE 3
...
static void fts_tcl_grip_mode_set(u8 mode);
...
static ssize_t fts_full_panel_init_show(struct device *dev,
				      struct device_attribute *attr, char *buf)
{
	struct fts_ts_info *info = dev_get_drvdata(dev);
	int res = 0, count = 0;
	...
	res = fts_disableInterrupt();
	if (res < OK) {
		count = snprintf(buf, PAGE_SIZE, "full panel init no ok\n");
		goto END;
	}
	...
	if (res < OK)
		...
		count = snprintf(buf, PAGE_SIZE, "full panel init ok\n");
...
static ssize_t fts_fod_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct fts_ts_info *info = dev_get_drvdata(dev);
...
static ssize_t fts_fod_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fts_ts_info *info = dev_get_drvdata(dev);
	u8 val;
...
static void tcl_grip_mode1_set(void)
{
	int ret;
	u8 corner0[12]={0xC0,0x0C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	u8 corner1[12]={0xC0,0x0C,0x00,0x01,0x38,0x04,0x00,0x00,0x38,0x04,0x00,0x00};
	u8 corner2[12]={0xC0,0x0C,0x00,0x02,0x00,0x00,0xDE,0x08,0x46,0x00,0x24,0X09};
	u8 corner3[12]={0xC0,0x0C,0x00,0x03,0xf2,0x03,0xDE,0x08,0x38,0x04,0x24,0x09};

	u8 corner_duration[7] = {0xC0,0x0D,0x00,0x01,0x00,0xFF,0xFF};

	u8 edge_top[12]    = {0xC0,0x0C,0x01,0x00,0x00,0x00,0x00,0x00,0x38,0x04,0x32,0x00};
	u8 edge_bottom[12] = {0xC0,0x0C,0x01,0x01,0x00,0x00,0xF2,0x08,0x38,0x04,0x24,0x09};
	u8 edge_left[12]   = {0xC0,0x0C,0x01,0x02,0x00,0x00,0x00,0x00,0x46,0x00,0x24,0x09};
	u8 edge_right[12]  = {0xC0,0x0C,0x01,0x03,0xF2,0x03,0x00,0x00,0x38,0x04,0x24,0x09};

	u8 edge_duration[7]   = {0xC0,0x0D,0x01,0x3C,0x00,0x01,0x00};

	u8 dead_top[12]    = {0xC0,0x0C,0x02,0x00,0x00,0x00,0x00,0x00,0x38,0x04,0x05,0x00};
	u8 dead_bottom[12] = {0xC0,0x0C,0x02,0x01,0x00,0x00,0x1F,0x09,0x38,0x04,0x24,0x09};
	u8 dead_left[12]   = {0xC0,0x0C,0x02,0x02,0x00,0x00,0x00,0x00,0x0A,0x00,0x24,0x09};
	u8 dead_right[12]  = {0xC0,0x0C,0x02,0x03,0x2E,0x04,0x00,0x00,0x38,0x04,0x24,0x09};

	ret = fts_write_dma_safe(corner0, sizeof(corner0) / sizeof(u8));
	ret = fts_write_dma_safe(corner1, sizeof(corner1) / sizeof(u8));
	ret = fts_write_dma_safe(corner2, sizeof(corner2) / sizeof(u8));
	ret = fts_write_dma_safe(corner3, sizeof(corner3) / sizeof(u8));

	ret = fts_write_dma_safe(edge_top, sizeof(edge_top) / sizeof(u8));
	ret = fts_write_dma_safe(edge_bottom, sizeof(edge_bottom) / sizeof(u8));
	ret = fts_write_dma_safe(edge_left, sizeof(edge_left) / sizeof(u8));
	ret = fts_write_dma_safe(edge_right, sizeof(edge_right) / sizeof(u8));

	ret = fts_write_dma_safe(dead_top, sizeof(dead_top) / sizeof(u8));
	ret = fts_write_dma_safe(dead_bottom, sizeof(dead_bottom) / sizeof(u8));
	ret = fts_write_dma_safe(dead_left, sizeof(dead_left) / sizeof(u8));
	ret = fts_write_dma_safe(dead_right, sizeof(dead_right) / sizeof(u8));

	ret = fts_write_dma_safe(edge_duration, sizeof(edge_duration) / sizeof(u8));
	ret = fts_write_dma_safe(corner_duration, sizeof(corner_duration) / sizeof(u8));

	pr_debug("[ FTS ] %s \n", __func__);
}

/* horizontal screen_1 setting*/
static void tcl_grip_mode2_set(void)
{
	int ret;
	u8 corner0[12]={0xC0,0x0C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	u8 corner1[12]={0xC0,0x0C,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	u8 corner2[12]={0xC0,0x0C,0x00,0x02,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	u8 corner3[12]={0xC0,0x0C,0x00,0x03,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};

	u8 corner_duration[7] = {0xC0,0x0D,0x00,0x01,0x00,0xFF,0xFF};

	u8 edge_top[12]    = {0xC0,0x0C,0x01,0x00,0xA2,0x03,0x00,0x00,0x38,0x04,0x96,0x00};
	u8 edge_bottom[12] = {0xC0,0x0C,0x01,0x01,0xA2,0x03,0x8E,0x08,0x38,0x04,0x24,0x09};
	u8 edge_left[12]   = {0xC0,0x0C,0x01,0x02,0x00,0x00,0x00,0x00,0x46,0x00,0x24,0x09};
	u8 edge_right[12]  = {0xC0,0x0C,0x01,0x03,0xF2,0x03,0x00,0x00,0x38,0x04,0x24,0x09};

	u8 edge_duration[7]   = {0xC0,0x0D,0x01,0x3C,0x00,0x01,0x00};

	u8 dead_top[12]    = {0xC0,0x0C,0x02,0x00,0x00,0x00,0x00,0x00,0x38,0x04,0x0F,0x00};
	u8 dead_bottom[12] = {0xC0,0x0C,0x02,0x01,0x00,0x00,0x15,0x09,0x38,0x04,0x24,0x09};
	u8 dead_left[12]   = {0xC0,0x0C,0x02,0x02,0x00,0x00,0x00,0x00,0x0A,0x00,0x24,0x09};
	u8 dead_right[12]  = {0xC0,0x0C,0x02,0x03,0x2E,0x04,0x00,0x00,0x38,0x04,0x24,0x09};
...
/* horizontal screen_2 setting*/
static void tcl_grip_mode3_set(void)
{
	int ret;
	u8 corner0[12]={0xC0,0x0C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	u8 corner1[12]={0xC0,0x0C,0x00,0x01,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	...
	u8 edge_top[12]    = {0xC0,0x0C,0x01,0x00,0x00,0x00,0x00,0x00,0x96,0x00,0x96,0x00};
	u8 edge_bottom[12] = {0xC0,0x0C,0x01,0x01,0x00,0x00,0x8E,0x08,0x96,0x00,0x24,0x09};
	u8 edge_left[12]   = {0xC0,0x0C,0x01,0x02,0x00,0x00,0x00,0x00,0x46,0x00,0x24,0x09};
	u8 edge_right[12]  = {0xC0,0x0C,0x01,0x03,0xF2,0x03,0x00,0x00,0x38,0x04,0x24,0x09};
...
/* google dialer setting*/
static void tcl_grip_google_dialer_set(void)
{
	int ret;
	u8 corner0[12]={0xC0,0x0C,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
	u8 corner1[12]={0xC0,0x0C,0x00,0x01,0x38,0x04,0x00,0x00,0x38,0x04,0x00,0x00};
	...
	u8 dead_top[12]    = {0xC0,0x0C,0x02,0x00,0x00,0x00,0xBF,0x03,0x3C,0x00,0xD5,0x06};
	u8 dead_bottom[12] = {0xC0,0x0C,0x02,0x01,0xFC,0x03,0xBF,0x03,0x38,0x04,0xD5,0x06};
	u8 dead_left[12]   = {0xC0,0x0C,0x02,0x02,0x00,0x00,0x00,0x00,0x0A,0x00,0x24,0x09};
	u8 dead_right[12]  = {0xC0,0x0C,0x02,0x03,0x2E,0x04,0x00,0x00,0x38,0x04,0x24,0x09};
...
static ssize_t fts_tcl_grip_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct fts_ts_info *info = dev_get_drvdata(dev);
...
static ssize_t fts_tcl_grip_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct fts_ts_info *info = dev_get_drvdata(dev);
	u8 val;
	...
	if (val & BIT(2))
		info->tcl_grip_mode = 4;
	else
		info->tcl_grip_mode = val & 0x03;

	pr_debug("%s val=%d, tcl_grip_mode=%d\n", __func__, val, info->tcl_grip_mode);

	fts_tcl_grip_mode_set(info->tcl_grip_mode);

	return count;
}

static ssize_t fts_ghost_fliter_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	u8 val;
	u8 cmd[3]={0xC0,0x02,0x00};
	int ret;

	kstrtou8(buf, 0, &val);

	if (val!=1 && val!=0)
		pr_err("%s error val=%d", __func__, val);
	else {
		cmd[2] = val;
		ret = fts_write_dma_safe(cmd, sizeof(cmd) / sizeof(u8));
	}
...
static DEVICE_ATTR(full_panel_init, (S_IRUGO | S_IWUSR | S_IWGRP), fts_full_panel_init_show, NULL);
static DEVICE_ATTR(fts_gesture, (S_IRUGO | S_IWUSR | S_IWGRP), fts_gesture_show, fts_gesture_store);
static DEVICE_ATTR(fts_fod, (S_IRUGO | S_IWUSR | S_IWGRP), fts_fod_show, fts_fod_store);
static DEVICE_ATTR(fts_tcl_grip, (S_IRUGO | S_IWUSR | S_IWGRP), fts_tcl_grip_show, fts_tcl_grip_store);
static DEVICE_ATTR(ghost_fliter, (S_IRUGO | S_IWUSR | S_IWGRP), NULL, fts_ghost_fliter_store);
...
#endif
	&dev_attr_full_panel_init.attr,
	&dev_attr_fts_gesture.attr,
	&dev_attr_fts_fod.attr,
	&dev_attr_fts_tcl_grip.attr,
	NULL,
};
...
*/
	...
	/*input_report_abs(info->input_dev, ABS_MT_TRACKING_ID, touchId); */
		...
		//input_report_abs(info->input_dev, ABS_MT_PRESSURE, p);
#ifdef CONFIG_FTS_FOD_AREA_REPORT
		if (fts_is_in_fodarea(x, y) && info->fod_status && !(info->fod_id & ~(1 << touchId))) {
			info->fod_x = x;
			info->fod_y = y;
					    ...
					    unsigned char *event)
{
	...
	if (info->touch_id == 0) {
		...
		if (info->fod_pressed == true)
			input_report_key(info->input_dev, KEY_GESTURE_L, 0);
		info->fod_pressed = false;
		//input_report_key(info->input_dev, BTN_INFO, 0);
		info->touch_skip = 0;
		info->sleep_finger = 0;
	...
	if (fod_up) {
		...
		logError(0, "%s  %s :  Event 0x%02x - release ID[%d] type = %d\n", tag,
				      ...
				      unsigned char *event)
{
	...
	int needCoords = 0;
	char ch[64] = { 0x0, };
#if 1//def CONFIG_FTS_FOD_AREA_REPORT
	int touch_area;
	int fod_overlap;
	int fod_id = 0;
...
#if 1//def CONFIG_FTS_FOD_AREA_REPORT
	logError(0,
		 "%s  gesture event data: %02X %02X %02X %02X %02X %02X %02X %02X x:%d y:%d\n",
		 tag, event[0], event[1], event[2], event[3], event[4],
		 event[5], event[6], event[7], x, y);
#else
	logError(1,
	...
	if (event[0] == EVT_ID_USER_REPORT && event[1] == EVT_TYPE_USER_GESTURE) {
		...
		if (event[2] == GEST_ID_LONG_PRESS) {
			...
			if (info->fod_pressed == false) {
				input_report_key(info->input_dev, KEY_GESTURE_L, 1);
				input_sync(info->input_dev);
				info->fod_pressed = true;
			}
	...
	if (fw_name) {
		...
		if ((retval & 0xFF000000) == ERROR_FLASH_PROCEDURE) {
			logError(1,
				 ...
				 tag, __func__, retval);
			fts_chip_powercycle(info);
			if (info->board->default_fw_name == NULL)
				logError(1, "%s not found default_fw_name!", tag);
			else
				logError(1, "%s default_fw_name = %s", tag, info->board->default_fw_name);
...
static void fts_fw_update_auto(struct work_struct *work)
{
	...
	fts_fw_update(info, "st_fts.ftb", 0);
}
#endif
...
static int fts_interrupt_install(struct fts_ts_info *info)
{
	...
	info->board->irq_flags = IRQF_TRIGGER_LOW | IRQF_ONESHOT;
...
static int fts_mode_handler(struct fts_ts_info *info, int force)
{
	...
	int ret = OK;
	u8 settings[4] = { 0 };
	u8 gesture_off_cmd[6] = {0xA2, 0x03, 0x00, 0x00, 0x00, 0x00};
	u8 double_tap_cmd[6] = {0xA2, 0x03, 0x20, 0x00, 0x00, 0x00};
	u8 single_double_cmd[6] = {0xA2, 0x03, 0x20, 0x00, 0x00, 0x03};
	u8 single_only_cmd[6] = {0xA2, 0x03, 0x00, 0x00, 0x00, 0x03};
#ifdef CONFIG_FTS_FOD_AREA_REPORT
	...
	case 0:
		if (info->fod_status) {
			res = fts_write_dma_safe(gesture_off_cmd, ARRAY_SIZE(gesture_off_cmd));
			if (res < OK)
					logError(1, "%s %s: gesture_off_cmd failed! ERROR %08X recovery in senseOff...\n",
						 tag, __func__, res);
			res = setScanMode(SCAN_MODE_LOW_POWER, 0);
			res |= ret;
			if (!info->gesture_enabled) {
				logError(1, "%s %s, fod mode\n", tag, __func__);
				res = fts_write_dma_safe(single_only_cmd, ARRAY_SIZE(single_only_cmd));
				if (res < OK)
						logError(1, "%s %s: set single_only_cmd failed! ERROR %08X\n", tag, __func__, res);
			} else {
				logError(1, "%s %s, fod&gesture mode\n", tag, __func__);
				res = fts_write_dma_safe(single_double_cmd, ARRAY_SIZE(single_double_cmd));
				if (res < OK)
						logError(1, "%s %s: set single_double_cmd failed! ERROR %08X\n", tag, __func__, res);
			}
			info->tp_status = FTS_LP_MODE;
		} else {
			logError(1, "%s %s: Sense OFF! \n", tag, __func__);
			ret = setScanMode(SCAN_MODE_ACTIVE, 0x00);
			res |= ret;
			info->tp_status = FTS_SUSPEND_MODE;
			if (info->gesture_enabled == 1) {
				logError(1, "%s %s, gesture mode\n", tag, __func__);
				res = fts_write_dma_safe(gesture_off_cmd, ARRAY_SIZE(gesture_off_cmd));
				if (res < OK)
						logError(1, "%s %s: gesture_off_cmd failed! ERROR %08X recovery in senseOff...\n",
							tag, __func__, res);
				res = setScanMode(SCAN_MODE_LOW_POWER, 0);
				res = fts_write_dma_safe(double_tap_cmd, ARRAY_SIZE(double_tap_cmd));
				if (res < OK)
						logError(1, "%s %s: double_tap_cmd failed! ERROR %08X recovery in senseOff...\n",
							tag, __func__, res);
				info->tp_status = FTS_LP_MODE;
			}
		}
	...
	case 1:
		...
		info->tp_status = FTS_NORMAL_MODE;
#endif
...
void fts_set_aod_mode(bool mode)
{
	//pr_info("[FTS]%s aod mode=%d, sensor_sleep=%d\n", __func__, mode, fts_info->sensor_sleep);

	fts_info->aod_status = mode;
	if (mode) {
		if (!fts_info->sensor_sleep) {
			logError(0, "%s %s: set tp off/lp mode\n", tag, __func__);
			flush_workqueue(fts_info->event_wq);
			queue_work(fts_info->event_wq, &fts_info->suspend_work);
		}
	} else {
		if (fts_info->sensor_sleep) {
			logError(0, "%s %s: set tp normal mode\n", tag, __func__);
			flush_workqueue(fts_info->event_wq);
...
static int fts_enable_reg(struct fts_ts_info *info, bool enable)
{
	...
	if (info->board->vdden_gpio >= 0) {
		gpio_direction_output(info->board->vdden_gpio, 1);
		logError(1, "%s %s: set vdden high\n", tag, __func__);
	}

	if (info->avdd_reg) {
		retval = regulator_set_voltage(info->avdd_reg, 3300000, 3300000);
		if (retval < 0) {
			logError(1, "%s %s: Failed to set power regulator voltage\n",
				 tag, __func__);
		}
...
static int parse_dt(struct device *dev, struct fts_hw_platform_data *bdata)
{
	...
	if (of_property_read_bool(np, "st,vdden-gpio")) {
		bdata->vdden_gpio = of_get_named_gpio_flags(np,
							    "st,vdden-gpio", 0,
							    NULL);
		logError(0, "%s vdden_gpio =%d\n", tag, bdata->vdden_gpio);
	} else
		bdata->vdden_gpio = GPIO_NOT_DEFINED;
	...
	if (retval < 0)
		bdata->default_fw_name = "st_fts.ftb";
...
static int fts_probe(struct spi_device *client)
{
	...
	if (info->board->vdden_gpio >= 0) {
		retval = gpio_request(info->board->vdden_gpio, "fts_vdden_gpio");
		if (retval) {
			logError(1, "%s %s: Failed to get gpio %d (code: %d)",
				 tag,
				 __func__, info->board->vdden_gpio, retval);
		}
	}
	...
	input_set_abs_params(info->input_dev, ABS_MT_WIDTH_MAJOR, AREA_MIN,
			     AREA_MAX, 0, 0);
	//input_set_abs_params(info->input_dev, ABS_MT_PRESSURE, 0, 255, 0, 0);
	...
	input_set_capability(info->input_dev, EV_KEY, KEY_GESTURE_D);
	input_set_capability(info->input_dev, EV_KEY, KEY_GESTURE_S);
	input_set_capability(info->input_dev, EV_KEY, KEY_GESTURE_L);
#endif
...
*/
	...
	if (error) {
		logError(1, "%s Failed to create sysfs group!\n", tag);
		goto ProbeErrorExit_8;
	}
#endif
	if (info->touch_panel_class == NULL)
		info->touch_panel_class = class_create(THIS_MODULE, "tp_device");

	if (info->touch_panel_class) {
		info->touch_panel_device =
			device_create(info->touch_panel_class, NULL, 0, NULL, "tp_device");
		if (info->touch_panel_device) {
			error = device_create_file(info->touch_panel_device, &dev_attr_ghost_fliter);
			if (error < 0)
				logError(1, "%s Failed to create file ghost_fliter!\n", tag);
		} else
			logError(1, "%s Failed to create touch_panel_device!\n", tag);
	}
	else
		logError(1, "%s Failed to create touch_panel_class!\n", tag);
