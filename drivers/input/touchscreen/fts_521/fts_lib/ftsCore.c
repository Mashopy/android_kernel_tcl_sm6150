		 ...
		 int time_to_wait)
{
	...
	while (find != 1 && retry < time_to_count
		...
		if (readData[0] == EVT_ID_ERROR) {
			logError(1, "%s ERROR EVENT = %02X %02X %02X %02X %02X %02X %02X %02X\n",
					tag, readData[0], readData[1], readData[2], readData[3],
			...
			if (readData[0] != EVT_ID_NOEVENT) {
				logError(0, "%s READ EVENT = %02X %02X %02X %02X %02X %02X %02X %02X \n",
					tag, readData[0], readData[1], readData[2], readData[3],
					readData[4], readData[5], readData[6], readData[7]);
			}
	...
	if ((retry >= time_to_count) && find != 1) {
		...
		logError(0, "%s FOUND EVENT = %02X %02X %02X %02X %02X %02X %02X %02X \n",
				tag, readData[0], readData[1], readData[2], readData[3],
				readData[4], readData[5], readData[6], readData[7]);
		logError(0,
