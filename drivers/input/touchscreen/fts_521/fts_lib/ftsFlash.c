...
int readFwFile(const char *path, Firmware *fw, int keep_cx)
{
	...
	if (path == NULL) {
		logError(1,
			 "%s readFwFile: fw file is null ... ERROR %08X\n",
			 tag, ERROR_MEMH_READ);
		return ERROR_MEMH_READ;
	}
...
int flash_burn(Firmware fw, int force_burn, int keep_cx)
{
	...
	if (!force_burn) {
		for (res = EXTERNAL_RELEASE_INFO_SIZE - 1; res >= 0; res--) {
			if (systemInfo.u8_releaseInfo[res] < fw.externalRelease[res]) {
				logError(1, "%s chip_release_info[%d]=%x, externalRelease[%d]=%x\n", 
					tag, res, systemInfo.u8_releaseInfo[res], res, fw.externalRelease[res]);
				goto start;
			}
		}
		logError(1,
