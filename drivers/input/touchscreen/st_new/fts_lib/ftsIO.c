...
int fts_read(u8 *outBuf, int byteToRead)
{
	 ...
	 */
	if (info->i2c_data_len < byteToRead + 1) {
		kfree(info->i2c_data);
		info->i2c_data = kmalloc(byteToRead + 1, GFP_KERNEL);
		if (!info->i2c_data) {
			info->i2c_data_len = 0;
			return -ENOMEM;
		}
		info->i2c_data_len = byteToRead + 1;
		buf = info->i2c_data;
	}
			   ...
			   readCmdLength, u8 *outBuf, int byteToRead)
{
	 ...
	 * Reassign memory for i2c_data in case length is greater than 32 bytes
	 */
	if (info->i2c_data_len < writeCmdLength + readCmdLength + byteToRead + 1) {
		kfree(info->i2c_data);
		info->i2c_data = kmalloc(writeCmdLength + readCmdLength + byteToRead + 1,
							GFP_KERNEL);
		if (!info->i2c_data) {
			info->i2c_data_len = 0;
			return -ENOMEM;
		}
		info->i2c_data_len = writeCmdLength + readCmdLength + byteToRead + 1;
		buf = info->i2c_data;
	}
