...
/* MODIFIED-BEGIN by ji.chen, 2019-07-01,BUG-7974483*/
#ifdef CONFIG_DRM
...
#include <linux/fb.h>
#endif
/* MODIFIED-END by ji.chen,BUG-7974483*/
...
int ff_drm_notifier_callback(struct notifier_block *self,
				unsigned long event, void *data)
{
    ...
    if ((event != MSM_DRM_EVENT_BLANK)) {
        FF_LOGV("event(%lu) do not need process\n", event);
        return 0;
    }

	blank = evdata->data;
	FF_LOGV("FB event:%lu,blank:%d", event, *blank);
...
#endif //CONFIG_DRM && CONFIG_FB // MODIFIED by ji.chen, 2019-07-01,BUG-7974483
...
static int ff_ctl_register_input(void)
{
    ...
    if (g_config) {
		...
		input_set_capability(g_context->input, EV_KEY, 250); // MODIFIED by ji.chen, 2019/08/03,PR-8058680
    }
...
/* MODIFIED-BEGIN by ji.chen, 2019-07-01,BUG-7974483*/
#ifdef CONFIG_DRM
    err = msm_drm_unregister_client(&g_context->fb_notifier);
#elif defined(CONFIG_FB)
...
static int ff_ctl_init_driver(void)
{
    ...
    //MOD-BEGIN by TCTNB.Ji.Chen,2019/12/03
    /* RST fingerprint at system startup. */
...
/* MODIFIED-BEGIN by ji.chen, 2019-07-01,BUG-7974483*/
#if defined(CONFIG_DRM)
    g_context->fb_notifier.notifier_call = ff_drm_notifier_callback;
    err = msm_drm_register_client(&g_context->fb_notifier);
#elif defined(CONFIG_FB)
